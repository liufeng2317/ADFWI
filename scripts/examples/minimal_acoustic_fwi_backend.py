#!/usr/bin/env python
"""Minimal acoustic FWI example using the bv1.2 backend API.

This is a script-style example for researchers who want to start from the
recommended one-line backend setup instead of editing notebook outputs. It builds
a tiny acoustic true/initial model pair, synthesizes observed data in memory,
runs one AcousticFWI iteration, and prints a JSON summary. It writes no files,
figures, notebooks, or wavefields.
"""
...
def model_arrays(nx: int, nz: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """Build small true/initial vp fields and a fixed rho field."""
...
def build_model(vp: np.ndarray, rho: np.ndarray, args: argparse.Namespace, *, vp_grad: bool) -> AcousticModel:
    """Create an AcousticModel that inherits the active ADFWI backend."""
...
def run_example(args: argparse.Namespace) -> Dict[str, Any]:
    """Run one tiny in-memory AcousticFWI iteration and return a JSON-ready summary."""
    ...
    true_model = build_model(true_vp, rho, args, vp_grad=False)
    ...
    model = build_model(init_vp, rho, args, vp_grad=True)
    ...
    if model.vp.grad is None:
        raise RuntimeError("model.vp.grad is None after one AcousticFWI iteration")
