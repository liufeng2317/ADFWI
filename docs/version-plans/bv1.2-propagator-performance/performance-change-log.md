...
| 2026-06-03 | `custom_chunk_strategy="remat_pressure_stride2"` acoustic wrapper opt-in | `ADFWI/propagator/acoustic_propagator.py`, `ADFWI/fwi/runtime/forward.py`, benchmark scripts | one-iteration wrapper gate loss diff `0.0`, raw `vp.grad` max abs diff `2.7381e-07`, total speedup `1.2339x`, memory ratio `2.2436x`; 5-iteration reduced FWI wrapper gate loss trajectory and `vp_update_norm` exact match, mean iteration `27.3676 s -> 22.8406 s`, memory ratio `2.2612x` | accepted expert opt-in, not default |
...
| 2026-06-03 | memory-budget remat full-divergence stride gate | `scripts/benchmark/acoustic_checkpoint_memory_matrix.py`, `scripts/benchmark/acoustic_experimental_fwi_loop_compare.py`, `acoustic_remat_puw_divergence_stride*_20260603.json` | `p,u,w` divergence cache stride=2 gave one-iteration speedup `1.2352x`, memory ratio `2.2436x`, loss diff `0.0`, raw `vp.grad` max abs diff `2.7381e-07`; 5-iteration reduced FWI loss trajectory and `vp_update_norm` matched exactly, mean iteration improved from `27.7475 s` to `22.9651 s`, memory ratio `2.2612x` | accepted as the current best budget-valid remat candidate; stride=3 is valid but slower |
| 2026-06-03 | remat stride=2 reverse-loop detail timing | temporary diagnostic only; code reverted after timing | manual adjoint step `11.4408 s` (`53.96%` backward), divergence recovery `2.3424 s` (`11.05%`), receiver scatter `0.9114 s` (`4.30%`), coefficient accumulation `0.3462 s` (`1.63%`) | next code-level work should target `_backward_step_from_saved_divergence`; do not revisit coefficient accumulation order |
| 2026-06-03 | manual adjoint repeated-slice local view cleanup | code reverted after one-iteration gate | loss diff `0.0`, raw `vp.grad` max abs diff `2.7381e-07`, memory ratio `2.2436x`, but total speedup `1.1989x` did not improve on the current stride=2 candidate | closed; slice-view cleanup is too small/noisy for the main line |
| 2026-06-03 | manual adjoint pressure-gradient clone removal | code reverted after one-iteration gate | loss diff `0.0`, raw `vp.grad` max abs diff `2.7381e-07`, memory ratio `2.2421x`, but total speedup `1.1940x` did not improve on the current stride=2 candidate | closed; clone/scatter micro-edits are too small/noisy for the main line |
| 2026-06-03 | full-record-geometry 10-iteration wrapper validation | `scripts/benchmark/acoustic_experimental_fwi_loop_compare.py`, `acoustic_production_remat_stride2_full_record_10iter_20260603.json` | `marmousi2_acoustic_full_record`, `shots=3`, `receivers=200`, `nt=3000`; max loss abs diff `4.8828125e-04`, `vp_update_norm` exact match, mean iteration `28.2326 s -> 23.7453 s`, total speedup `1.1890x`, memory ratio `2.2612x` | accepted as longer wrapper validation; next validation should increase shot count or iteration count |
