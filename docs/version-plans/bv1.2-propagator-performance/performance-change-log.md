...
| 2026-06-03 | checkpoint=1 speed/memory upper-bound matrix | `scripts/benchmark/acoustic_checkpoint_memory_matrix.py`, `acoustic_checkpoint_memory_matrix_observed_reduced_20260603.json` | same reduced observed-pressure gate: production checkpoint=1 total speedup `1.2087x`, peak memory `7.4425x`; production custom chunk total speedup `1.3435x`, peak memory `27.1292x`; loss/output parity exact and raw `vp.grad` finite | checkpoint=1 is faster but not enough to justify unbounded memory; next route should target checkpoint=1-like speed with memory closer to checkpoint=1 than saved-state custom chunk |
