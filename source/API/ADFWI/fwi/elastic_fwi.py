...
class ElasticFWI(torch.nn.Module):
    ...
    propagator : ElasticPropagator
        The propagator used for simulating elastic wave propagation.
    model : AbstractModel
        The model class representing the velocity structure.
    loss_fn : Union[Misfit, torch.autograd.Function]
        The loss function used to compute the misfit between observed and predicted data.
    obs_data : SeismicData
        The observed seismic data.
    optimizer : Union[torch.optim.Optimizer, List[torch.optim.Optimizer]], optional
        The optimizer or list of optimizers for model parameters. Default is None.
    scheduler : torch.optim.lr_scheduler, optional
        The learning rate scheduler for optimizing the model parameters. Default is None.
    gradient_processor : Union[GradProcessor, List[GradProcessor]], optional
        Processor(s) for handling gradients (e.g., vp/vs/rho, epsilon/delta/gamma). Default is None.
    regularization_fn : Optional[Regularization], optional
        Regularization function(s) applied to parameters like vp/vs/rho/epsilon/delta/gamma. Default is None.
    regularization_weights_x : Optional[List[Union[float]]], optional
        Regularization weights for the x-axis. Default is [0, 0, 0, 0, 0, 0].
    regularization_weights_z : Optional[List[Union[float]]], optional
        Regularization weights for the z-axis. Default is [0, 0, 0, 0, 0, 0].
    waveform_normalize : Optional[bool], optional
        Whether to normalize the waveforms during inversion. Default is True.
    ...
    cache_result : Optional[bool], optional
        Whether to save intermediate results during the inversion. Default is True.
    cache_gradient : Optional[bool], optional
        Whether to save model variations (not gradients) during inversion. Default is False.
    save_fig_epoch : Optional[int], optional
        The interval (in epochs) at which to save the inversion result figure. Default is -1 (no figure saved).
    save_fig_path : Optional[str], optional
        The path where to save the inversion result figure. Default is an empty string (no save path).
    inversion_component : Optional[np.array], optional
        The components of the inversion (e.g., ["pressure"]). Default is ["pressure"].
    """
    ...
    def forward(self,
        ...
        fd_order : int, optional
            The order of the finite difference scheme for wave propagation. Default is 4.
