...
class ElasticPropagator(torch.nn.Module):
    ...
    -----------
    model : AbstractModel
        The model object, which contains the properties of the physical model (e.g., velocity, density).
    survey : Survey
        The survey object, which contains information about the source and receiver locations and wavelet.
    device : Optional[str], default='cpu'
        The device to run the computation on. Can be 'cpu' for CPU or 'cuda' for GPU.
    cpu_num : Optional[int], default=1
        The number of CPU cores to use for parallel computations.
    gpu_num : Optional[int], default=1
        The number of GPUs to use for parallel computations. Should be set according to available GPU resources.
    dtype : torch.dtype, default=torch.float32
        The data type of the tensors. Typically torch.float32 for balance between performance and precision.
    """
    def __init__(self,
                model:AbstractModel,
                survey:Survey,
    ...
    def forward(self,
        ...
        model : Optional[AbstractModel]
            The model for the simulation. If None, use the default model.
        shot_index : Optional[int]
            Index of the shot for which to compute the waveform.
        fd_order : int
            The order of the finite difference method (default is 4).
        checkpoint_segments : int
            The number of segments for saving intermediate states (default is 1).
        
        Returns
        --------
        record_waveform: torch.Tensor
            The recorded waveforms at the receiver locations.
        """
