...
class Tikhonov_2order(Regularization):
    """2nd-Order Tikhonov Regularization
        math: 
            ||alphax*L2 m_x + alphaz*L2 m_z||_2 
    ...
    """
    def __init__(self,nx,nz,dx,dz,alphax=0,alphaz=0,step_size=1000,gamma=1,device=None,dtype=None) -> None:
        ...
        L0[0,:]  = 0
        L0[-1,:] = 0
        self.L0 = numpy2tensor(L0,dtype=self.dtype).to(self.device)
        # horizontal constraint
        L1 = np.diag(1*np.ones(nx-1),1) + np.diag(1*np.ones(nx-1),-1) + np.diag(-2*np.ones(nx))
        L1[0,:]  = 0
        L1[-1,:] = 0
        self.L1 = numpy2tensor(L1,dtype=self.dtype).to(self.device)
