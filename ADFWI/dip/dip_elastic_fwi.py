    ...
    def process_gradient(self, parameter,forw,idx=None):
        with torch.no_grad():
            ...
            if isinstance(self.gradient_processor, GradProcessor):
                grads = self.gradient_processor.forward(nz=self.model.nz, nx=self.model.nx, vmax=vmax, grad=grads, forw=forw_p if "pressure" in self.inversion_component else forw_vz)
            else:
                grads = self.gradient_processor[idx].forward(nz=self.model.nz, nx=self.model.nx, vmax=vmax, grad=grads, forw=forw_p if "pressure" in self.inversion_component else forw_vz)
        ...
        for i in pbar_epoch:
            ...
            def grad_post_process(grads, parameter, forw=None, idx=None):
                ...
                with torch.no_grad():
                    ...
                    # Apply gradient processor
                    if isinstance(self.gradient_processor, GradProcessor):
                        grads = self.gradient_processor.forward(nz=self.propagator.model.nz, nx=self.propagator.model.nx, vmax=vmax, grad=grads, forw=forw_p if "pressure" in self.inversion_component else forw_vz)
                    else:
                        grads = self.gradient_processor[idx].forward(nz=self.propagator.model.nz, nx=self.propagator.model.nx, vmax=vmax, grad=grads, forw=forw_p if "pressure" in self.inversion_component else forw_vz)
            ...
            if self.propagator.model.get_requires_grad("vp"):
                self.propagator.model.vp.register_hook(lambda grad: grad_post_process(grad, "vp", forw=forw_p if "pressure" in self.inversion_component else forw_vz, idx=0))
            if self.propagator.model.get_requires_grad("vs"):
                self.propagator.model.vs.register_hook(lambda grad: grad_post_process(grad, "vs", forw=forw_p if "pressure" in self.inversion_component else forw_vz, idx=1))
            if self.propagator.model.get_requires_grad("rho"):
                self.propagator.model.rho.register_hook(lambda grad: grad_post_process(grad, "rho", forw=forw_p if "pressure" in self.inversion_component else forw_vz, idx=2))
            if isinstance(self.model, (DIP_VTIModel,DIP_VTIModel_o4a)):
                if self.propagator.model.get_requires_grad("eps"):
                    self.propagator.model.eps.register_hook(lambda grad: grad_post_process(grad, "eps", forw=forw_p if "pressure" in self.inversion_component else forw_vz, idx=3))
                if self.propagator.model.get_requires_grad("delta"):
                    self.propagator.model.delta.register_hook(lambda grad: grad_post_process(grad, "delta", forw=forw_p if "pressure" in self.inversion_component else forw_vz, idx=4))
                if self.propagator.model.get_requires_grad("gamma"):
                    self.propagator.model.gamma.register_hook(lambda grad: grad_post_process(grad, "gamma", forw=forw_p if "pressure" in self.inversion_component else forw_vz, idx=5))
