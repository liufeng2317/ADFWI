...
class MLPs(torch.nn.Module):
    def __init__(self,model_shape,
        ...
        self.MLP_branches = nn.ModuleList([
            ...
            ) for _ in range(self.out_channels_number)
        ])
    ...
    def forward(self):
        ...
        for i in range(self.out_channels_number):
            out_temp = self.MLP_branches[i](out).view(self.model_shape[0], self.model_shape[1])
            vmin, vmax, unit = self.vmins[i], self.vmaxs[i], self.units[i]
            if vmin is not None and vmax is not None:
                out_temp = ((vmax - vmin) * torch.tanh(out_temp) + (vmax + vmin)) / 2
            out_res.append(out_temp * unit)
