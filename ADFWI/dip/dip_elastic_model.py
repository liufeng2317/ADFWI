...
class DIP_ElasticModel(AbstractModel):
    """Elastic Velocity model using re-parameterized vp, vs and rho
    """
    def __init__(self,
                ox:float,oz:float,
                nx:int  ,nz:int,
                ...
                DIP_model_vs                                     = None,     # deep image prior models
                DIP_model_rho                                    = None,
        ...
        DIP_model_vs                             : reparameterized vs using a deep neural network, by default None
        ...
        if self.reparameterization_inputs == "vel": # input the reference velocity model
            self._parameterization_thomson(self.vp_init.unsqueeze(0).unsqueeze(0))
            ...
            self._parameterization_thomson(self.coords)
        else:
    ...
    def _parameterization_thomson(self,*args,**kw_args):
        ...
        if self.DIP_model_vs is not None:
            if self.reparameterization_strategy == "vel":
                ...
                self.vs = self.vs_init + self.DIP_model_vs(*args,**kw_args)
    ...
    def forward(self,*args,**kwargs) -> Tuple:
        ...
        if self.reparameterization_inputs == "vel":
            ...
            self._parameterization_thomson(numpy2tensor(self.get_model("vp")).to(self.device).unsqueeze(0).unsqueeze(0))
