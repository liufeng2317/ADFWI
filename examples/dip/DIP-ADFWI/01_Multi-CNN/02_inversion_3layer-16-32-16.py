...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(project_path,f"inversion-{layer_num}layer-16-32-16"), exist_ok=True)
    ...
    # -----------------------------------
    model_shape = [nz,nx]
    DIP_model = DIP_CNN(model_shape,in_channels=[16,32,16],vmin=vp_true.min()/1000,vmax=vp_true.max()/1000)
    DIP_model.to(device)
    ...
    torch.save(model.DIP_model_vp.state_dict(),os.path.join(project_path,f"inversion-{layer_num}layer-16-32-16/DIP_model.pt"))
