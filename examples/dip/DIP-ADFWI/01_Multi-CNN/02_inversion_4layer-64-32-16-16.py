...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(project_path,f"inversion-{layer_num}layer-64-32-16-16"), exist_ok=True)
    ...
    # -----------------------------------
    model_shape = [nz,nx]
    DIP_model = DIP_CNN(model_shape,in_channels=[64,32,16,16],vmin=vp_true.min()/1000,vmax=vp_true.max()/1000)
    DIP_model.to(device)
