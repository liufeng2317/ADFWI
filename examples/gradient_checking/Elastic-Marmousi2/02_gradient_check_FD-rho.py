...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-FD/rho")):
        os.makedirs(os.path.join(project_path,"inversion-FD/rho"))
    ...
    for i in tqdm(range(nx), desc='Outer loop'):
        for j in tqdm(range(nz), desc='Inner loop', leave=False):
            rho_init_temp = rho_init.copy()
            rho_init_temp[i,j] = rho_init_temp[i,j] + 1
            
            model = IsotropicElasticModel(
                    ox,oz,nx,nz,dx,dz,
                    vp_init,vs_init,rho_init_temp,
                    vp_grad = False,vs_grad = False, rho_grad=False,
                    auto_update_rho=False,auto_update_vp=False,
                    free_surface=free_surface,
                    abc_type="PML",abc_jerjan_alpha=0.007,nabc=nabc,
            ...
            np.savez(os.path.join(project_path, f"inversion-FD/rho/obs_{i}_{j}.npz"),data=rcv_p.cpu().detach().numpy())
    ...
    for i in tqdm(range(nx)):
        for j in range(nz):
            perturbation_csg = np.load("./data/inversion-FD/rho/obs_{}_{}.npz".format(i,j))["data"]
            perturbation_grad[i][j] = np.sum((syn_csg_p - perturbation_csg)*(syn_csg_p-obs_csg_p))/1

    np.savez(os.path.join(project_path,"inversion-FD/rho/grad_rho.npz"),data = perturbation_grad)