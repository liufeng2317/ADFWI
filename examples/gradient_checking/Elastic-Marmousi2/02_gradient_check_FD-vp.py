...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-FD/vp")):
        os.makedirs(os.path.join(project_path,"inversion-FD/vp"))
    ...
    for i in tqdm(range(nx), desc='Outer loop'):
        for j in tqdm(range(nz), desc='Inner loop', leave=False):
            ...
            np.savez(os.path.join(project_path, f"inversion-FD/vp/obs_{i}_{j}.npz"),data=rcv_p.cpu().detach().numpy())
    ...
    for i in tqdm(range(nx)):
        for j in range(nz):
            perturbation_csg = np.load("./data/inversion-FD/vp/obs_{}_{}.npz".format(i,j))["data"]
            perturbation_grad[i][j] = np.sum((syn_csg_p - perturbation_csg)*(syn_csg_p-obs_csg_p))/1

    np.savez(os.path.join(project_path,"inversion-FD/vp/grad_vp.npz"),data = perturbation_grad)