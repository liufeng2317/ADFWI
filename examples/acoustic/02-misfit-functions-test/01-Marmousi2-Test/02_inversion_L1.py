...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-L1")):
        os.makedirs(os.path.join(project_path,"inversion-L1"))
    ...
    fwi = AcousticFWI(propagator=F,
                    ...
                    cache_result=True,  
                    save_fig_epoch=50,  
                    save_fig_path=os.path.join(project_path, "inversion-L1"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,"inversion-L1/misfit.png"),show=False)
    
    # inverted results
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,"inversion-L1/inverted_res.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, "inversion-L1/inversion_process.gif"),fps=10)