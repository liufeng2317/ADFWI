...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(project_path,"inversion-AdamW"), exist_ok=True)
    ...
    # optimizer
    iteration   =   300
    optimizer   =   torch.optim.AdamW(model.parameters(), lr=10,betas=(0.9,0.999), weight_decay=1e-6)
    ...
    # inverted results
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,"inversion-AdamW/inverted_res.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, "inversion-AdamW/inversion_process.gif"),fps=10)