...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-LBFGS")):
        os.makedirs(os.path.join(project_path,"inversion-LBFGS"))
    ...
    # loss_fn = Misfit_waveform_L2(dt=10)
    # loss_fn = Misfit_global_correlation(dt=10)

    # gradient processor
    grad_mask = np.ones_like(vp_init)
    grad_mask[:10,:] = 0
    gradient_processor = GradProcessor(grad_mask=grad_mask,forw_illumination=True,norm_grad=False)
    ...
    fwi = AcousticFWI(propagator=F,
                    ...
                    cache_result=True,  
                    save_fig_epoch=1,  
                    save_fig_path=os.path.join(project_path, "inversion-LBFGS"))

    fwi.forward(iteration=iteration,batch_size=None,checkpoint_segments=1)
    
    iter_vp     = fwi.iter_vp
    iter_loss   = fwi.iter_loss 
    np.savez(os.path.join(project_path,"inversion-LBFGS/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,"inversion-LBFGS/iter_loss.npz"),data=np.array(iter_loss))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,"inversion-LBFGS/misfit.png"),show=False)
    
    # inverted results
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,"inversion-LBFGS/inverted_res.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, "inversion-LBFGS/inversion_process.gif"),fps=10)