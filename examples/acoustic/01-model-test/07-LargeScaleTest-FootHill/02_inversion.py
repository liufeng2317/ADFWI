...
if __name__ == "__main__":
    ...
    nz, nx = 290, 800         # Grid dimensions in z and x directions
    dx, dz = 20, 20           # Grid spacing in x and z directions
    nt, dt = 4000, 0.0015      # Time steps and time interval
    nabc   = 30               # Thickness of the absorbing boundary layer
    f0     = 6                # Initial frequency in Hz
    ...
    vp_true      = np.loadtxt(os.path.join(model_path,"Foothill_801_331_25m.dat"))[:nx,:nz]
    ...
    src_z = np.array([1 for i in range(1, nx-1, 20)])  # Z-coordinates for sources
    src_x = np.array([i for i in range(1, nx-1, 20)])  # X-coordinates for sources
    ...
    # inverted results
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,"inversion/inverted_res.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, "inversion/inversion_process.gif"),fps=10)