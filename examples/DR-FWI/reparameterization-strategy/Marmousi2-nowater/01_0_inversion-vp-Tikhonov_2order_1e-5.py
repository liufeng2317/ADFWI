...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"GC/inversion-vp-tikhonov2_1e-5")):
        os.makedirs(os.path.join(project_path,"GC/inversion-vp-tikhonov2_1e-5"))
    ...
    fwi = AcousticFWI(propagator=F,
                        ...
                        regularization_weights_x=[1e-5,0],
                        regularization_weights_z=[1e-5,0],
                        obs_data=d_obs,
                        gradient_processor=gradient_processor,
                        waveform_normalize=True,
                        cache_result=True,
                        save_fig_epoch=50,
                        save_fig_path=os.path.join(project_path,f"GC/inversion-vp-tikhonov2_1e-5")
                        )
    ...
    np.savez(os.path.join(project_path,f"GC/inversion-vp-tikhonov2_1e-5/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"GC/inversion-vp-tikhonov2_1e-5/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"GC/inversion-vp-tikhonov2_1e-5/iter_loss.npz"),data=np.array(iter_loss))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"GC/inversion-vp-tikhonov2_1e-5/misfit.png"),show=False)
    
    # inverted results
    vp_init = vp_init.cpu().detach().numpy() if torch.is_tensor(vp_init) else vp_init
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"GC/inversion-vp-tikhonov2_1e-5/inverted_vp.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"GC/inversion-vp-tikhonov2_1e-5/inversion_vp.gif"),fps=10)