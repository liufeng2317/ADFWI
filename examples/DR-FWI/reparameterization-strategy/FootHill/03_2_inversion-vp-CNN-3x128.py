...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(project_path,"statistic/inversion-vp-CNN-3x128"), exist_ok=True)
    ...
    nt, dt = 2001, 0.003      # Time steps and time interval
    ...
    model_path = str(next(parent for parent in SCRIPT_DIR.parents if parent.name == "examples") / "datasets" / "foothill_source")

    vp_true  = np.loadtxt(os.path.join(model_path,"Foothill_801_331_25m.dat"))[:801,:291]
    vp_init = np.copy(vp_true)
    vp_init = smooth2d(vp_true, span = 25)
    vp_init[vp_true[:,:]==1500]=1500

    vp_true     = vp_true[::3,::3].T
    vp_init     = vp_init[::3,::3].T
    ...
    # vp_init  = np.copy(vp_true)
    # vp_init  = smooth2d(vp_true, span = 6)
    # rho_init = np.power(vp_init, 0.25) * 310
    # mask = vp_true == 1500
    # vp_init[mask]  = 1500
    # rho_init[mask] = 1000
    ...
    if pretrain:
        if load_pretrained:
            ...
            DIP_model_vp.load_state_dict(torch.load(os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/DIP_model_vp_pretrained.pt")))
        else:
            lr          = 0.0005
            iteration   = 10000
            step_size   = 1000
            ...
            torch.save(DIP_model_vp.state_dict(),os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/DIP_model_vp_pretrained.pt"))

    water_layer_mask = vp_true == 1500
    ...
    #------------------------------------------------------
    # Source
    src_z = np.array([1 for i in range(1, nx, 5)])  # Z-coordinates for sources
    src_x = np.array([i for i in range(1, nx, 5)])  # X-coordinates for sources
    ...
    # F.boundary_condition(vmax=2000)
    damp = F.damp
    ...
    fwi = DIP_AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=50,
                        save_fig_path=os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128")
                        )
    ...
    np.savez(os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model_vp.state_dict(),os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/DIP_model.pt"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/misfit.png"),show=False)
    
    # inverted results
    vp_init = vp_init.cpu().detach().numpy() if torch.is_tensor(vp_init) else vp_init
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"statistic/inversion-vp-CNN-3x128/inverted_vp.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"statistic/inversion-vp-CNN-3x128/inversion_vp.gif"),fps=10)