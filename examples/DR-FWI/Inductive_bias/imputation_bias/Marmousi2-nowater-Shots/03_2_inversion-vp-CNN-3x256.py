...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-vp-CNN-3x256")):
        os.makedirs(os.path.join(project_path,"inversion-vp-CNN-3x256"))
    ...
    if pretrain:
        if load_pretrained:
            ...
            DIP_model_vp.load_state_dict(torch.load(os.path.join(project_path,f"inversion-vp-CNN-3x256/DIP_model_vp_pretrained.pt")))
        else:
            lr          = 0.0005
            iteration   = 10000
            step_size   = 1000
            ...
            torch.save(DIP_model_vp.state_dict(),os.path.join(project_path,f"inversion-vp-CNN-3x256/DIP_model_vp_pretrained.pt"))
    ...
    elif settings["shot"] == 20:
        # 20 sources
        src_z = np.array([1 for i in range(5, nx-1, 10)])  # Z-coordinates for sources
        src_x = np.array([i for i in range(5, nx-1, 10)])  # X-coordinates for sources
    elif settings["shot"] == 10:
        # 10 sources
        src_z = np.array([1 for i in range(10, nx-1, 20)])  # Z-coordinates for sources
        src_x = np.array([i for i in range(10, nx-1, 20)])  # X-coordinates for sources
    elif settings["shot"] == 5:
        # 5 sources
        src_z = np.array([1 for i in range(20, nx-1, 40)])  # Z-coordinates for sources
        src_x = np.array([i for i in range(20, nx-1, 40)])  # X-coordinates for sources
    elif settings["shot"] == 2:
        # 2 sources
        src_z = np.array([1 for i in range(60, nx-1, 80)])  # Z-coordinates for sources
        src_x = np.array([i for i in range(60, nx-1, 80)])  # X-coordinates for sources
    ...
    fwi = DIP_AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=50,
                        save_fig_path=os.path.join(project_path,f"inversion-vp-CNN-3x256")
                        )

    fwi.forward(iteration=iteration,batch_size=None,checkpoint_segments=3)
    
    iter_vp     = fwi.iter_vp
    iter_rho    = fwi.iter_rho
    iter_loss   = fwi.iter_loss 
    np.savez(os.path.join(project_path,f"inversion-vp-CNN-3x256/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp-CNN-3x256/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp-CNN-3x256/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model_vp.state_dict(),os.path.join(project_path,f"inversion-vp-CNN-3x256/DIP_model.pt"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp-CNN-3x256/misfit.png"),show=False)
    
    # inverted results
    vp_init = vp_init.cpu().detach().numpy() if torch.is_tensor(vp_init) else vp_init
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp-CNN-3x256/inverted_vp.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp-CNN-3x256/inversion_vp.gif"),fps=10)