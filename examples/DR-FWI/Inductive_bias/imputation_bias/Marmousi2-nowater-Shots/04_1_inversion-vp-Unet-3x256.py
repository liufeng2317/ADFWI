...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-vp-Unet-3x256")):
        os.makedirs(os.path.join(project_path,"inversion-vp-Unet-3x256"))
    ...
    if pretrain:
        if load_pretrained:
            ...
            DIP_model_vp.load_state_dict(torch.load(os.path.join(project_path,f"inversion-vp-Unet-3x256/DIP_model_vp_pretrained.pt")))
        else:
            lr          = 0.0005
            iteration   = 10000
            step_size   = 1000
            ...
            torch.save(DIP_model_vp.state_dict(),os.path.join(project_path,f"inversion-vp-Unet-3x256/DIP_model_vp_pretrained.pt"))
    ...
    fwi = DIP_AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=50,
                        save_fig_path=os.path.join(project_path,f"inversion-vp-Unet-3x256")
                        )
    ...
    np.savez(os.path.join(project_path,f"inversion-vp-Unet-3x256/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp-Unet-3x256/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp-Unet-3x256/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model_vp.state_dict(),os.path.join(project_path,f"inversion-vp-Unet-3x256/DIP_model.pt"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp-Unet-3x256/misfit.png"),show=False)
    
    # inverted results
    vp_init = vp_init.cpu().detach().numpy() if torch.is_tensor(vp_init) else vp_init
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp-Unet-3x256/inverted_vp.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp-Unet-3x256/inversion_vp.gif"),fps=10)