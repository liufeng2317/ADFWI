...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-vp-baseline")):
        os.makedirs(os.path.join(project_path,"inversion-vp-baseline"))
    ...
    fwi = AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=50,
                        save_fig_path=os.path.join(project_path,f"inversion-vp-baseline")
                        )
    ...
    np.savez(os.path.join(project_path,f"inversion-vp-baseline/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp-baseline/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp-baseline/iter_loss.npz"),data=np.array(iter_loss))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp-baseline/misfit.png"),show=False)
    
    # inverted results
    vp_init = vp_init.cpu().detach().numpy() if torch.is_tensor(vp_init) else vp_init
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp-baseline/inverted_vp.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp-baseline/inversion_vp.gif"),fps=10)