...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,"inversion-vp-dv-MLP-2x100")):
        os.makedirs(os.path.join(project_path,"inversion-vp-dv-MLP-2x100"))
    ...
    fwi = DIP_AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=100,
                        save_fig_path=os.path.join(project_path,f"inversion-vp-dv-MLP-2x100")
                        )
    ...
    np.savez(os.path.join(project_path,f"inversion-vp-dv-MLP-2x100/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp-dv-MLP-2x100/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp-dv-MLP-2x100/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model_vp.state_dict(),os.path.join(project_path,f"inversion-vp-dv-MLP-2x100/DIP_model.pt"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp-dv-MLP-2x100/misfit.png"),show=False)
    
    # inverted results
    vp_init = vp_init.cpu().detach().numpy() if torch.is_tensor(vp_init) else vp_init
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp-dv-MLP-2x100/inverted_vp.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp-dv-MLP-2x100/inversion_vp.gif"),fps=10)