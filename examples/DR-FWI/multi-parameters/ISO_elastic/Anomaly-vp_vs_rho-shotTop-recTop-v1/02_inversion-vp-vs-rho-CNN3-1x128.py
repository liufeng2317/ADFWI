...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128")):
        os.makedirs(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128"))
    #------------------------------------------------------
    #                   Basic Parameters
    #------------------------------------------------------
    device = "cuda:1"
    ...
    if pretrain:
        if load_pretrained:
            ...
            DIP_model.load_state_dict(torch.load(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/DIP_model_vp_vs_rho_pretrained.pt")))
        else:
            lr          = 0.0001
            iteration   = 2000
            step_size   = 1000
            ...
            torch.save(DIP_model.state_dict(),os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/DIP_model_vp_vs_rho_pretrained.pt"))
    ...
    fwi = DIP_ElasticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=10,
                        save_fig_path=os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128"),
    ...
    np.savez(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/iter_vs.npz"),data=np.array(iter_vs))
    np.savez(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model.state_dict(),os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/DIP_model_vp_vs_rho.pt"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/misfit.png"),show=False)
    ...
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/inverted_vp.png"),show=False)
    plot_initial_and_inverted(vp_init=vs_init,iter_vp=iter_vs,save_path=os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/inverted_vs.png"),show=False)
    plot_initial_and_inverted(vp_init=rho_init,iter_vp=iter_rho,save_path=os.path.join(project_path,f"inversion-vp_vs_rho-CNN3-1x128/inverted_rho.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp_vs_rho-CNN3-1x128/inversion_vp.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_vs,vmin=vs_true.min(),vmax=vs_true.max(),save_path=os.path.join(project_path, f"inversion-vp_vs_rho-CNN3-1x128/inversion_vs.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_rho,vmin=rho_true.min(),vmax=rho_true.max(),save_path=os.path.join(project_path, f"inversion-vp_vs_rho-CNN3-1x128/inversion_rho.gif"),fps=10)