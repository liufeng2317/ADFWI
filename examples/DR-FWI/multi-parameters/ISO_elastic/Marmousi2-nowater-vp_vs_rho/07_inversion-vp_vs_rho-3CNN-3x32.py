...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32")):
        os.makedirs(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32"))
    #------------------------------------------------------
    #                   Basic Parameters
    #------------------------------------------------------
    device = "cuda:0"
    ...
    # -----------------------------------
    model_shape  = [nz,nx]
    DIP_model_vp = DIP_CNN(model_shape,in_channels=[32,32,32],vmin=vp_true.min()/1000 ,vmax=vp_true.max()/1000 ,device=device)
    DIP_model_vs = DIP_CNN(model_shape,in_channels=[32,32,32],vmin=vs_true.min()/1000,vmax=vs_true.max()/1000,device=device)
    DIP_model_rho= DIP_CNN(model_shape,in_channels=[32,32,32],vmin=rho_true.min()/1000,vmax=rho_true.max()/1000,device=device)
    ...
    if pretrain:
        if load_pretrained:
            ...
            DIP_model_vp.load_state_dict(torch.load(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_vp_pretrained.pt")))
        else:
            lr          = 0.0005
            iteration   = 10000
            step_size   = 1000
            ...
            torch.save(DIP_model_vp.state_dict(),os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_vp_pretrained.pt"))

    if pretrain:
        if load_pretrained:
            # load the model parameters
            DIP_model_vs.load_state_dict(torch.load(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_vs_pretrained.pt")))
        else:
            lr          = 0.0005
            iteration   = 10000
            step_size   = 1000
            ...
            for i in pbar:  
                ...
                scheduler.step()
                pbar.set_description(f'Pretrain Iter:{i}, Misfit:{loss.cpu().detach().numpy()}')
            torch.save(DIP_model_vs.state_dict(),os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_vs_pretrained.pt"))
    
    if pretrain:
        if load_pretrained:
            # load the model parameters
            DIP_model_rho.load_state_dict(torch.load(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_rho_pretrained.pt")))
        else:
            lr          = 0.0005
            iteration   = 10000
            step_size   = 1000
            ...
            for i in pbar:  
                ...
                scheduler.step()
                pbar.set_description(f'Pretrain Iter:{i}, Misfit:{loss.cpu().detach().numpy()}')
            torch.save(DIP_model_rho.state_dict(),os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_rho_pretrained.pt"))
    # -----------------------------------
    ...
    fwi = DIP_ElasticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=10,
                        save_fig_path=os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32"),
    ...
    np.savez(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/iter_vs.npz"),data=np.array(iter_vs))
    np.savez(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model_vp.state_dict(),os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_vp.pt"))
    torch.save(model.DIP_model_vs.state_dict(),os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_vs.pt"))
    torch.save(model.DIP_model_rho.state_dict(),os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/DIP_model_rho.pt"))
    #------------------------------------------------------
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/misfit.png"),show=False)
    ...
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/inverted_vp.png"),show=False)
    plot_initial_and_inverted(vp_init=vs_init,iter_vp=iter_vs,save_path=os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/inverted_vs.png"),show=False)
    plot_initial_and_inverted(vp_init=rho_init,iter_vp=iter_rho,save_path=os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/inverted_rho.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/inversion_vp.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_vs,vmin=vs_true.min(),vmax=vs_true.max(),save_path=os.path.join(project_path, f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/inversion_vs.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_rho,vmin=rho_true.min(),vmax=rho_true.max(),save_path=os.path.join(project_path, f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x32/inversion_rho.gif"),fps=10)