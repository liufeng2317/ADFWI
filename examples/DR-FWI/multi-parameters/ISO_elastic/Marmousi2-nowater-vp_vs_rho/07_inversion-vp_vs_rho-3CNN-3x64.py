...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(project_path,f"no-gradient-smooth/inversion-vp_vs_rho-CNN-3x64"), exist_ok=True)
    #------------------------------------------------------
    #                   Basic Parameters
    #------------------------------------------------------
    device = "npu:0"
    ...
    # -----------------------------------
    model_shape  = [nz,nx]
    DIP_model_vp = DIP_CNN(model_shape,in_channels=[64,64,64],vmin=vp_true.min()/1000 ,vmax=vp_true.max()/1000 )
    DIP_model_vs = DIP_CNN(model_shape,in_channels=[64,64,64],vmin=vs_true.min()/1000,vmax=vs_true.max()/1000)
    DIP_model_rho= DIP_CNN(model_shape,in_channels=[64,64,64],vmin=rho_true.min()/1000,vmax=rho_true.max()/1000)
