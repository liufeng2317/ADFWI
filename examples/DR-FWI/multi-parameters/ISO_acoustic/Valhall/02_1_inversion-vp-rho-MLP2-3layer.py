...
from ADFWI.dip import DIP_AcousticFWI, DIP_MLP
...
if __name__ == "__main__":
    ...
    os.makedirs(os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer"), exist_ok=True)
    ...
    model_shape   = [nz,nx]
    DIP_model  = DIP_MLP(model_shape,random_state_num=100,hidden_layer_number=[100,100,100], out_channels_number=2,
                        vmins=[vp_true.min()/1000,rho_true.min()/1000],vmaxs=[vp_true.max()/1000,rho_true.max()/1000],
                        units=[1000,1000])
    DIP_model.to(device)
    ...
    if pretrain:
        if load_pretrained:
            ...
            DIP_model.load_state_dict(torch.load(os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/DIP_model_vp_rho_pretrained.pt")))
        else:
            lr          = 0.001
            iteration   = 10000
            step_size   = 1000
            ...
            for i in pbar:  
                vp_nn,rho_nn = DIP_model()
                loss = torch.sqrt(torch.sum((vp_nn - vp_init)**2)) + torch.sqrt(torch.sum((rho_nn - rho_init)**2))
                optimizer.zero_grad()
                loss.backward()
                optimizer.step()
                scheduler.step()
                pbar.set_description(f'Pretrain Iter:{i}, Misfit:{loss.cpu().detach().numpy()}')
            torch.save(DIP_model.state_dict(),os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/DIP_model_vp_rho_pretrained.pt"))
    ...
    fwi = DIP_AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=100,
                        save_fig_path=os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer")
                        )
    ...
    np.savez(os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/iter_loss.npz"),data=np.array(iter_loss))
    torch.save(model.DIP_model.state_dict(),os.path.join(project_path,f"inversion-vp-rho-MLP2-2layer/DIP_model_vp_rho.pt"))
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/misfit.png"),show=False)
    ...
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/inverted_vp.png"),show=False)
    plot_initial_and_inverted(vp_init=rho_init,iter_vp=iter_rho,save_path=os.path.join(project_path,f"inversion-vp-rho-MLP2-3layer/inverted_rho.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp-rho-MLP2-3layer/inversion_vp.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_rho,vmin=rho_true.min(),vmax=rho_true.max(),save_path=os.path.join(project_path, f"inversion-vp-rho-MLP2-3layer/inversion_rho.gif"),fps=10)