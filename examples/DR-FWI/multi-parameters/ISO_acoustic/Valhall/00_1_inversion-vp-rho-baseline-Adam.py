...
if __name__ == "__main__":
    ...
    if not os.path.exists(os.path.join(project_path,f"inversion-vp-rho-baseline")):
        os.makedirs(os.path.join(project_path,f"inversion-vp-rho-baseline"))
    ...
    fwi = AcousticFWI(propagator=F,
                        ...
                        cache_result=True,
                        save_fig_epoch=100,
                        save_fig_path=os.path.join(project_path,f"inversion-vp-rho-baseline")
                        )

    fwi.forward(iteration=iteration,batch_size=None,checkpoint_segments=10)
    
    iter_vp     = fwi.iter_vp
    iter_rho    = fwi.iter_rho
    iter_loss   = fwi.iter_loss 
    np.savez(os.path.join(project_path,f"inversion-vp-rho-baseline/iter_vp.npz"),data=np.array(iter_vp))
    np.savez(os.path.join(project_path,f"inversion-vp-rho-baseline/iter_rho.npz"),data=np.array(iter_rho))
    np.savez(os.path.join(project_path,f"inversion-vp-rho-baseline/iter_loss.npz"),data=np.array(iter_loss))
    #------------------------------------------------------
    ...
    # misfit
    plot_misfit(iter_loss = iter_loss, save_path=os.path.join(project_path,f"inversion-vp-rho-baseline/misfit.png"),show=False)
    ...
    plot_initial_and_inverted(vp_init=vp_init,iter_vp=iter_vp,save_path=os.path.join(project_path,f"inversion-vp-rho-baseline/inverted_vp.png"),show=False)
    plot_initial_and_inverted(vp_init=rho_init,iter_vp=iter_rho,save_path=os.path.join(project_path,f"inversion-vp-rho-baseline/inverted_rho.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_vp,vmin=vp_true.min(),vmax=vp_true.max(),save_path=os.path.join(project_path, f"inversion-vp-rho-baseline/inversion_vp.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_rho,vmin=rho_true.min(),vmax=rho_true.max(),save_path=os.path.join(project_path, f"inversion-vp-rho-baseline/inversion_rho.gif"),fps=10)