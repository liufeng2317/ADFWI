...
if __name__ == "__main__":
    ...
    plot_initial_and_inverted(vp_init=delta_init,iter_vp=iter_delta,save_path=os.path.join(project_path,"inversion/inverted_delta.png"),show=False)
    
    # inversion animation
    animate_inversion_process(iter_vp=iter_eps,vmin=epsilon_true.min(),vmax=epsilon_true.max(),save_path=os.path.join(project_path, "inversion/inversion_eps.gif"),fps=10)
    animate_inversion_process(iter_vp=iter_delta,vmin=delta_true.min(),vmax=delta_true.max(),save_path=os.path.join(project_path, "inversion/inversion_delta.gif"),fps=10)